-- session_recaps stores the on-demand LLM recap of a session (PCC-241): a
...
-- session's count climbs past it the console prompts for an updated recap.
-- A recap whose turn_count still matches the session is immutable, so repeat
...
-- or workflow heuristic) extracted by the same LLM pass — a recap is a chance
...
CREATE TABLE IF NOT EXISTS session_recaps (
    ...
    CONSTRAINT session_recaps_pkey PRIMARY KEY (org_id, session_id)
);
