...
var _ = Describe("v1 session handlers", func() {
	...
	BeforeEach(func() {
		...
		var err error
		server, err = NewServer(Config{ListenAddr: ":0"}, inMem, inMem, logger)
	...
	Describe("GET /v1/sessions", func() {
		...
		Context("with a root and three leaves", func() {
			...
			BeforeEach(func() {
				...
				root = merkle.NewNode(v1TestBucket("user", "kickoff", "claude-opus-4-6", "anthropic", "claude"), nil, merkle.NodeMeta{Project: "tapes"})
				root.CreatedAt = baseTime
				Expect(putNode(ctx, inMem, root)).To(Succeed())

				leafC = merkle.NewNode(v1TestBucket("assistant", "oldest answer", "claude-opus-4-6", "anthropic", "claude"), root, merkle.NodeMeta{Project: "tapes"})
				leafC.CreatedAt = baseTime.Add(1 * time.Minute)
				Expect(putNode(ctx, inMem, leafC)).To(Succeed())

				leafB = merkle.NewNode(v1TestBucket("assistant", "middle answer", "claude-sonnet-4-6", "anthropic", "opencode"), root, merkle.NodeMeta{Project: "other"})
				leafB.CreatedAt = baseTime.Add(2 * time.Minute)
				Expect(putNode(ctx, inMem, leafB)).To(Succeed())

				leafA = merkle.NewNode(v1TestBucket("assistant", "newest answer", "gpt-4o", "openai", "claude"), root, merkle.NodeMeta{Project: "tapes"})
	...
	Describe("GET /v1/stats", func() {
		BeforeEach(func() {
			root := merkle.NewNode(v1TestBucket("user", "root", "m", "p", "claude"), nil, merkle.NodeMeta{Project: "tapes"})
			Expect(putNode(ctx, inMem, root)).To(Succeed())

			leafA := merkle.NewNode(v1TestBucket("assistant", "a", "m", "p", "claude"), root, merkle.NodeMeta{Project: "tapes"})
			leafB := merkle.NewNode(v1TestBucket("assistant", "b", "m", "p", "claude"), root, merkle.NodeMeta{Project: "other"})
			Expect(putNode(ctx, inMem, leafA)).To(Succeed())
			Expect(putNode(ctx, inMem, leafB)).To(Succeed())
		})
