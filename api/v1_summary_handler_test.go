...
var _ = Describe("GET /v1/sessions/summary", func() {
	...
	BeforeEach(func() {
		...
		var err error
		server, err = NewServer(cfg, inMem, inMem, logger)
	...
	seedSession := func(tag string, offset time.Duration, inputTokens, outputTokens int, stop string) *merkle.Node {
		...
		user := merkle.NewNode(userBucket, nil, merkle.NodeMeta{Project: "tapes"})
		...
		assistant := merkle.NewNode(assistantBucket, user, merkle.NodeMeta{
			Project:    "tapes",
			StopReason: stop,
			Usage: &llm.Usage{
