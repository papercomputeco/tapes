...
func NewServer(config Config, driver storage.Driver, log *slog.Logger) (*Server, error) {
	...
	// Session reflection (PCC-241): POST generates (or returns the stored reflection
	// when the session hasn't accrued turns since), GET reads it back.
	app.Post("/v1/sessions/:id/reflection", s.handleGenerateSessionReflection)
	app.Get("/v1/sessions/:id/reflection", s.handleGetSessionReflection)
