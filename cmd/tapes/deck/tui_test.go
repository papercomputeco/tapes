...
var _ = Describe("Deck TUI helpers", func() {
	...
	Describe("analytics day helpers", func() {
		buildDays := func(count int) []deck.DayActivity {
			days := make([]deck.DayActivity, 0, count)
			for i := 1; i <= count; i++ {
				days = append(days, deck.DayActivity{Date: time.Date(2026, 2, i, 0, 0, 0, 0, time.Local).Format("2006-01-02")})
			}
			return days
		}

		Describe("analyticsSelectableDays", func() {
			It("returns empty for no days", func() {
				Expect(analyticsSelectableDays(nil)).To(BeEmpty())
			})

			It("returns all days when under limit", func() {
				days := buildDays(5)
				selected := analyticsSelectableDays(days)
				Expect(selected).To(HaveLen(5))
				Expect(selected[0].Date).To(Equal(days[0].Date))
				Expect(selected[4].Date).To(Equal(days[4].Date))
			})

			It("returns last nine days", func() {
				days := buildDays(12)
				selected := analyticsSelectableDays(days)
				Expect(selected).To(HaveLen(9))
				Expect(selected[0].Date).To(Equal(days[3].Date))
				Expect(selected[8].Date).To(Equal(days[11].Date))
			})
		})

		Describe("analyticsDayExists", func() {
			It("reports when a day exists", func() {
				days := buildDays(3)
				Expect(analyticsDayExists(days, days[1].Date)).To(BeTrue())
				Expect(analyticsDayExists(days, "2026-02-10")).To(BeFalse())
			})
		})

		Describe("trimDateLabel", func() {
			It("trims the year prefix", func() {
				Expect(trimDateLabel("2026-02-13")).To(Equal("02-13"))
				Expect(trimDateLabel("02-13")).To(Equal("02-13"))
			})
		})

		Describe("parseAnalyticsDay", func() {
			It("returns a full-day range in local time", func() {
				rangeResult, err := parseAnalyticsDay("2026-02-13")
				Expect(err).ToNot(HaveOccurred())
				Expect(rangeResult.from.Year()).To(Equal(2026))
				Expect(rangeResult.from.Month()).To(Equal(time.February))
				Expect(rangeResult.from.Day()).To(Equal(13))
				Expect(rangeResult.from.Hour()).To(Equal(0))
				Expect(rangeResult.to.Sub(rangeResult.from)).To(Equal(24 * time.Hour))
			})
		})

		Describe("selectAnalyticsDay", func() {
			It("returns false when analytics are missing", func() {
				model := deckModel{}
				_, ok := model.selectAnalyticsDay("1")
				Expect(ok).To(BeFalse())
			})

			It("selects from the last nine days", func() {
				days := buildDays(12)
				model := deckModel{analytics: &deck.AnalyticsOverview{ActivityByDay: days}}
				selected, ok := model.selectAnalyticsDay("1")
				Expect(ok).To(BeTrue())
				Expect(selected).To(Equal(days[3].Date))
			})
		})
	})
})
