...
var _ = Describe("spawnMenu", func() {
	...
	It("returns nil on non-darwin platforms", func() {
		if runtime.GOOS == "darwin" {
			Skip("test only applicable on non-darwin platforms")
		}
		manager, err := start.NewManager(tmpDir)
		Expect(err).NotTo(HaveOccurred())

		cmder := &startCommander{configDir: tmpDir}
		cmd := cmder.spawnMenu(manager, newDiscardLogger())
		Expect(cmd).To(BeNil())
	})

	It("propagates debug and config-dir flags into args", func() {
		if runtime.GOOS != "darwin" {
			Skip("spawnMenu only runs on darwin")
		}
		// Use a state file so killStaleMenu has something to read (no stale PID).
		manager, err := start.NewManager(tmpDir)
		Expect(err).NotTo(HaveOccurred())
		...
		// The binary being spawned (our own test binary) will fail immediately
		// because "menu" isn't a valid test subcommand, but spawnMenu returns
		// a non-nil cmd as long as Start() succeeds.
		cmder := &startCommander{configDir: tmpDir, debug: true}
		cmd := cmder.spawnMenu(manager, newDiscardLogger())
		// The executable may or may not start successfully depending on the
		// binary — if it does, clean up; if not, that's fine too.
		if cmd != nil && cmd.Process != nil {
			_ = cmd.Process.Kill()

			// Verify PID was saved to state.
			loaded, loadErr := manager.LoadState()
			Expect(loadErr).NotTo(HaveOccurred())
			Expect(loaded.MenuPID).To(Equal(cmd.Process.Pid))
		}
	})
})

var _ = Describe("killStaleMenu", func() {
	var tmpDir string

	BeforeEach(func() {
		var err error
		tmpDir, err = os.MkdirTemp("", "tapes-kill-menu-*")
		Expect(err).NotTo(HaveOccurred())
	})
	...
	It("does nothing when no state file exists", func() {
		manager, err := start.NewManager(tmpDir)
		Expect(err).NotTo(HaveOccurred())

		cmder := &startCommander{configDir: tmpDir}
		Expect(func() {
			cmder.killStaleMenu(manager, newDiscardLogger())
		}).NotTo(Panic())
	})

	It("does nothing when MenuPID is zero", func() {
		manager, err := start.NewManager(tmpDir)
		Expect(err).NotTo(HaveOccurred())
		Expect(manager.SaveState(&start.State{
			DaemonPID: os.Getpid(),
	...
	It("does nothing when MenuPID refers to a dead process", func() {
		manager, err := start.NewManager(tmpDir)
		Expect(err).NotTo(HaveOccurred())
		Expect(manager.SaveState(&start.State{
			DaemonPID: os.Getpid(),
	...
	It("terminates a live stale menu process", func() {
		// Start a long-running sleep process to act as the stale menu.
		sleepCmd := exec.Command("sleep", "60")
		Expect(sleepCmd.Start()).To(Succeed())

		waitDone := make(chan error, 1)
		go func() { waitDone <- sleepCmd.Wait() }()
		...
		// The sleep process should exit after receiving SIGTERM.
		Eventually(waitDone, 5*time.Second).Should(Receive())
	})
})
...
var _ = Describe("saveMenuPID", func() {
	...
	It("persists the menu PID in state", func() {
		manager, err := start.NewManager(tmpDir)
		Expect(err).NotTo(HaveOccurred())
		Expect(manager.SaveState(&start.State{
			DaemonPID: os.Getpid(),
		...
		cmder := &startCommander{configDir: tmpDir}
		cmder.saveMenuPID(manager, 42, newDiscardLogger())
	...
	It("does not panic when no state exists", func() {
		manager, err := start.NewManager(tmpDir)
		Expect(err).NotTo(HaveOccurred())
	...
	It("preserves existing state fields", func() {
		...
		cmder := &startCommander{configDir: tmpDir}
		cmder.saveMenuPID(manager, 55, newDiscardLogger())
