...
// SessionReflectionRecord is the flat session_reflections-table row surfaced by the
// /v1/sessions/{id}/reflection API (PCC-241): the on-demand LLM narrative of a
// session, one row per session (regeneration upserts, latest wins).
type SessionReflectionRecord struct {
	SessionID string
	...
	// LLM pass — a reflection is a chance for an observation. Empty when the pass
	...
	// time. A reflection whose TurnCount still matches the session is current
	...
	// for an update.
	TurnCount int
	// Model is the LLM that authored the reflection.
	Model       string
