// Package recap extracts a human-facing session recap from a session
...
// observation — a recap is a chance for an observation.
//
...
// call, so recap generation needs no plumbing of its own.
package recap
...
// recap reads one session — and its opening (the goal) and ending (the
...
// Recap is the LLM's extraction: the narrative plus an optional observation.
type Recap struct {
...
func (g *Generator) Generate(ctx context.Context, sessionID string, live bool) (*Recap, error) {
	...
	basePrompt := buildRecapPrompt(transcript, live)
	...
	for attempt := range maxGenerateRetries {
		...
		recap, err := parseRecapResponse(response)
		if err != nil {
...
// the middle — for a recap, the session's opening (goal) and ending (outcome)
// carry the signal.
...
func buildRecapPrompt(transcript string, live bool) string {
	...
	return fmt.Sprintf(`Analyze the following LLM coding session transcript and write a recap.
...
func parseRecapResponse(response string) (*Recap, error) {
	jsonStr := response
	...
	if err := json.Unmarshal([]byte(jsonStr), &recap); err != nil {
		return nil, fmt.Errorf("unmarshal recap JSON: %w", err)
	}
	recap.Narrative = strings.TrimSpace(recap.Narrative)
	recap.Observation = strings.TrimSpace(recap.Observation)
	if recap.Narrative == "" {
		return nil, errors.New("recap narrative is empty")
	}
	return &recap, nil
}
