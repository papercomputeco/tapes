...
var _ = Describe("Vertex Provider", func() {
	...
	Describe("ParseRequest", func() {
		Context("with a simple text request", func() {
			It("parses messages correctly", func() {
				...
				Expect(req.Extra).To(HaveKeyWithValue("anthropic_version", "vertex-2023-10-16"))
			})
		})

		Context("with model in the body", func() {
			It("parses the model field when present", func() {
				payload := []byte(`{
					"anthropic_version": "vertex-2023-10-16",
		...
		Context("without model in the body", func() {
			It("parses successfully with empty model", func() {
				payload := []byte(`{
					"anthropic_version": "vertex-2023-10-16",
					"max_tokens": 256,
					"messages": [
		...
		Context("with system prompt", func() {
			It("parses a string system field", func() {
				payload := []byte(`{
					"anthropic_version": "vertex-2023-10-16",
					"max_tokens": 1024,
			...
			It("parses an array system field", func() {
				payload := []byte(`{
					"anthropic_version": "vertex-2023-10-16",
					"max_tokens": 1024,
					"system": [
		...
		Context("preserves raw request", func() {
			It("stores the original payload in RawRequest", func() {
				payload := []byte(`{"anthropic_version": "vertex-2023-10-16", "max_tokens": 1024, "messages": []}`)
				req, err := p.ParseRequest(payload)
				Expect(err).NotTo(HaveOccurred())
