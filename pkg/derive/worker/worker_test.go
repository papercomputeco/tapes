...
var _ = Describe("Worker", func() {
	...
	Describe("span embed pass", func() {
		...
		embedConfig := func() worker.Config {
			cfg := fastConfig()
			cfg.SpanEmbed = worker.SpanEmbedFunc(func(context.Context) error {
				embedMu.Lock()
				defer embedMu.Unlock()
				embedRuns++
				return embedErr
		...
		BeforeEach(func() {
			embedMu.Lock()
			embedRuns, embedErr = 0, nil
			embedMu.Unlock()
		})

		It("runs once at startup and again after a poll that derived", func() {
			store.mark(settled("org-a", "claude-code", "sess-1"))

			startWorker(embedConfig())

			// Startup backlog pass, then the post-derive pass.
			Eventually(store.deriveCount).Should(Equal(1))
			Eventually(embedCount).Should(Equal(2))

			// Idle polls (nothing derived) never re-trigger it.
			Consistently(embedCount, "50ms").Should(Equal(2))
		})

		It("absorbs embed failures without stalling derivation", func() {
			embedMu.Lock()
			embedErr = errors.New("embedding backend down")
			embedMu.Unlock()
			store.mark(settled("org-a", "claude-code", "sess-1"))
			...
			Eventually(func() bool {
				_, ok := store.entry("org-a", "claude-code", "sess-1")
				return ok
			}).Should(BeFalse(), "the derive must clear even when the embed pass fails")
		})
	})
})
